 ...
 * CLEAN VERSION - Ready to copy to WordPress
 */
...
// Shows: Est. Year, Yoga Alliance, Google, Yelp, Verified
// ============================================
...
function ynm_trust_badges_shortcode($atts) {
    ...
    // Get data from GeoDirectory - try multiple possible field names
    // Verified = claimed listing in GeoDirectory
    ...
    // Yoga Alliance - try common field names
    ...
    if (empty($yoga_alliance)) $yoga_alliance = geodir_get_post_meta($post_id, 'certification', true);

    // Established year - try common field names
    ...
    if (empty($established)) $established = geodir_get_post_meta($post_id, 'founded', true);

    // Google rating
    $google_rating = geodir_get_post_meta($post_id, 'google_rating', true);
    if (empty($google_rating)) $google_rating = geodir_get_post_meta($post_id, 'googlerating', true);
    $google_reviews = geodir_get_post_meta($post_id, 'google_reviews', true);
    if (empty($google_reviews)) $google_reviews = geodir_get_post_meta($post_id, 'googlereviews', true);

    // Yelp rating
    $yelp_rating = geodir_get_post_meta($post_id, 'yelp_rating', true);
    if (empty($yelp_rating)) $yelp_rating = geodir_get_post_meta($post_id, 'yelprating', true);
    $yelp_reviews = geodir_get_post_meta($post_id, 'yelp_reviews', true);
    if (empty($yelp_reviews)) $yelp_reviews = geodir_get_post_meta($post_id, 'yelpreviews', true);

    // Fallback to gd_post object (check claimed first, then verified)
    ...
    ?>
    <style>
    .ynm-trust-badges{display:flex;flex-wrap:wrap;align-items:center;gap:10px;margin:8px 0 12px 0}
    ...
    <div class="ynm-trust-badges">
        ...
        <span class="ynm-trust-badge established">
        ...
        <span class="ynm-trust-badge yoga-alliance">
            <i class="fas fa-om"></i>
            Yoga Alliance <?php echo esc_html($yoga_alliance); ?>
        </span>
        <?php endif; ?>
        ...
        <span class="ynm-trust-badge google">
            ...
            <?php if (!empty($google_reviews)): ?>
                <span class="rating-count">(<?php echo esc_html($google_reviews); ?>)</span>
            <?php endif; ?>
        </span>
        <?php endif; ?>
        ...
        <span class="ynm-trust-badge yelp">
            ...
            <?php if (!empty($yelp_reviews)): ?>
                <span class="rating-count">(<?php echo esc_html($yelp_reviews); ?>)</span>
            <?php endif; ?>
        </span>
        <?php endif; ?>
        ...
        <span class="ynm-trust-badge featured">
