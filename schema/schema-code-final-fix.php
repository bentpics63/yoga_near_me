<?php
/**
 * Yoga Near Me - Studio Schema Markup (FINAL FIX - With Output Buffering)
 * 
 * This version uses:
 * 1. Multiple filters to remove Rank Math's LocalBusiness schema
 * 2. Output buffering as a last resort to strip LocalBusiness from HTML
 * 3. Custom YogaStudio schema output
 * 
 * INSTRUCTIONS:
 * 1. Replace ALL existing schema code in your functions.php with this version
 * 2. Save the file
...
// ============================================
// METHOD 2: Output buffering to remove LocalBusiness from HTML (last resort)
// ============================================
...
function ynm_remove_localbusiness_from_output($buffer) {
    ...
    // Handles escaped slashes in JSON and multiline content
    $pattern = '/<script[^>]*type\s*=\s*["\']application\/ld\+json["\'][^>]*>\s*\{[^<]*"@type"\s*:\s*"LocalBusiness"[^<]*<\/script>/is';
    
    // Also try a more flexible pattern for multiline JSON
    $pattern2 = '/<script[^>]*type\s*=\s*["\']application\/ld\+json["\'][^>]*>[\s\S]*?"@type"\s*:\s*"LocalBusiness"[\s\S]*?<\/script>/i';
    
    // Remove all LocalBusiness schema blocks (try both patterns)
