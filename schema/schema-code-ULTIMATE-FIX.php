<?php
/**
 * Yoga Near Me - Studio Schema Markup (ULTIMATE FIX)
 * 
 * This version uses MULTIPLE methods to ensure Rank Math's LocalBusiness schema is removed:
 * 1. Multiple Rank Math filter hooks (comprehensive coverage)
 * 2. Disable Rank Math schema generation entirely for GeoDirectory posts
 * 3. Output buffering with robust regex patterns (catches everything)
 * 4. Custom YogaStudio schema output
 * 
 * INSTRUCTIONS:
 * 1. BACKUP your current functions.php
 * 2. Remove ALL existing schema code from functions.php (search for "ynm_" functions)
 * 3. Copy ALL code below into your theme's functions.php
 * 4. Save the file
 * 5. Clear ALL caches:
 ...
 * 7. View page source and search for "ld+json" - should see YogaStudio, NOT LocalBusiness
 */

// ============================================
// METHOD 1: Multiple Rank Math Filter Hooks
// ============================================

/**
 * Filter 1: Remove LocalBusiness from rank_math/json_ld (primary filter)
 */
...
/**
 * Filter 2: Remove from validated_data (backup filter)
 */
...
/**
 * Filter 3: Remove LocalBusiness from schema array before processing
 * This runs early to catch it before other filters
 */
function ynm_early_remove_localbusiness($schemas) {
    if (function_exists('geodir_is_page') && geodir_is_page('detail')) {
        // Remove LocalBusiness but keep other schemas (BreadcrumbList, ImageObject, etc.)
...
add_filter('rank_math/json_ld', 'ynm_early_remove_localbusiness', 1, 1);

/**
 * Filter 4: Remove LocalBusiness from schema data before it's processed
 */
function ynm_remove_localbusiness_from_schema_data($data, $json) {
    if (function_exists('geodir_is_page') && geodir_is_page('detail')) {
        if (isset($data['LocalBusiness'])) {
            unset($data['LocalBusiness']);
        }
        if (is_array($data)) {
            foreach ($data as $key => $value) {
                if (is_array($value) && isset($value['@type']) && $value['@type'] === 'LocalBusiness') {
                    unset($data[$key]);
                }
            }
        }
...
add_filter('rank_math/json_ld', 'ynm_remove_localbusiness_from_schema_data', 998, 2);

// ============================================
// METHOD 2: Output Buffering (Last Resort)
// ============================================

/**
 * Remove LocalBusiness schema from final HTML output using output buffering
 * This catches ANY LocalBusiness schema that filters missed
 * Uses multiple regex patterns to catch all variations
 */
function ynm_remove_localbusiness_from_output($buffer) {
    // Only process on single GeoDirectory listing pages
    if (!function_exists('geodir_is_page') || !geodir_is_page('detail')) {
        return $buffer;
    }
    
    // Pattern 1: Match single-line LocalBusiness schema
    // Matches: <script type="application/ld+json">{"@context":"...","@type":"LocalBusiness",...}</script>
    $pattern1 = '/<script[^>]*type\s*=\s*["\']application\/ld\+json["\'][^>]*>\s*\{[^<]*"@type"\s*:\s*"LocalBusiness"[^<]*<\/script>/is';
    
    // Pattern 2: Match multiline LocalBusiness schema (more flexible)
    // Matches: <script...>...{"@type":"LocalBusiness"...}...</script>
    $pattern2 = '/<script[^>]*type\s*=\s*["\']application\/ld\+json["\'][^>]*>[\s\S]*?"@type"\s*:\s*"LocalBusiness"[\s\S]*?<\/script>/i';
    
    // Pattern 3: Match with escaped slashes in JSON
    // Matches: <script...>{"@context":"https:\/\/schema.org","@type":"LocalBusiness"...}</script>
    $pattern3 = '/<script[^>]*type\s*=\s*["\']application\/ld\+json["\'][^>]*>[\s\S]*?"@type"\s*:\s*"LocalBusiness"[\s\S]*?<\/script>/i';
    
    // Pattern 4: Match even if there are whitespace/newlines between script tag and JSON
    $pattern4 = '/<script[^>]*type\s*=\s*["\']application\/ld\+json["\'][^>]*>[\s\S]*?"@type"\s*:\s*"LocalBusiness"[\s\S]*?<\/script>/is';
    
    // Remove all LocalBusiness schema blocks (try all patterns)
    $buffer = preg_replace($pattern1, '', $buffer);
    $buffer = preg_replace($pattern2, '', $buffer);
    $buffer = preg_replace($pattern3, '', $buffer);
    $buffer = preg_replace($pattern4, '', $buffer);
    
    return $buffer;
}

// Start output buffering on template_redirect (runs at the right time)
