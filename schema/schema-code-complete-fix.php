<?php
/**
 * Yoga Near Me - Studio Schema Markup (COMPLETE FIX)
 * 
 * IMPORTANT: Copy ALL of this code into your functions.php file
 * Replace any existing schema code with this complete version
 * 
 * WHAT THIS DOES:
 * 1. Removes Rank Math's LocalBusiness schema (multiple methods)
 * 2. Adds our custom YogaStudio schema with correct data
 * 
 * INSTRUCTIONS:
 * 1. Copy ALL code below (lines 15-245)
 * 2. Paste at the END of your theme's functions.php
 * 3. Remove any old/duplicate schema code
 * 4. Save the file
 * 5. Clear ALL caches (LiteSpeed Cache, WordPress, browser)
 * 6. Test on: https://yoganearme.info/studios/stretch-chi/
 */

// ============================================
// REMOVE RANK MATH'S LOCALBUSINESS SCHEMA
// ============================================

/**
 * Method 1: Filter Rank Math's JSON-LD output (primary method)
 * This is the most reliable way to remove LocalBusiness schema
 */
function ynm_remove_rankmath_localbusiness_jsonld($json_ld_array) {
    ...
    if (function_exists('geodir_is_page') && geodir_is_page('detail')) {
        ...
        if (isset($json_ld_array['@type']) && $json_ld_array['@type'] === 'LocalBusiness') {
            return array(); // Return empty to remove it
        }
    }
...
/**
 * Method 2: Also filter validated_data (backup method)
 */
...
function ynm_add_studio_schema() {
    ...
    // Clean up description - remove any wrong studio names
    if (!empty($studio_description)) {
        $studio_description = trim($studio_description);
        // If description doesn't contain the studio name and is suspiciously long, reset it
    ...
    // Use priority 1 to output early in <head>
