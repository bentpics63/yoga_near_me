<?php
/**
 * Yoga Near Me - Studio Schema Markup (SAFE FIX - No Critical Errors)
 * 
 * This version is safer and includes error checking to prevent fatal errors.
 * 
 * INSTRUCTIONS:
 * 1. REMOVE the problematic code from functions.php (the one causing critical error)
...
function ynm_remove_rankmath_localbusiness_jsonld($json_ld_array) {
    // Safety check: Only run if GeoDirectory is active
...
function ynm_remove_localbusiness_from_output($buffer) {
    // Safety check: Make sure buffer is a string
    if (!is_string($buffer)) {
        return $buffer;
    }
    
    // We already checked geodir_is_page when starting the buffer,
    // so we can process the buffer directly here
    
    // Multiple patterns to catch all variations of LocalBusiness schema
    // Pattern 1: Standard format with escaped slashes
    $pattern1 = '/<script[^>]*type\s*=\s*["\']application\/ld\+json["\'][^>]*>[\s\S]*?"@type"\s*:\s*"LocalBusiness"[\s\S]*?<\/script>/i';
    
    // Pattern 2: Handle escaped slashes in JSON
    $pattern2 = '/<script[^>]*type\s*=\s*["\']application\/ld\+json["\'][^>]*>[\s\S]*?"@type"\s*:\s*["\']LocalBusiness["\'][\s\S]*?<\/script>/i';
    
    // Pattern 3: More aggressive - find any script with LocalBusiness anywhere
    $pattern3 = '/<script[^>]*>[\s\S]*?"@type"\s*:\s*"LocalBusiness"[\s\S]*?<\/script>/i';
    
    // Pattern 4: Even more aggressive - match with escaped quotes
    $pattern4 = '/<script[^>]*>[\s\S]*?@type[\s\S]*?LocalBusiness[\s\S]*?<\/script>/i';
    
    // Remove LocalBusiness schema blocks (try all patterns)
...
add_action('template_redirect', function() {
    ...
    // Safety check: Only if GeoDirectory is active
    if (!function_exists('geodir_is_page')) {
        return;
    }
    ...
    // Only on single GeoDirectory listing pages
    if (geodir_is_page('detail')) {
        // Check if output buffering is already active (avoid conflicts)
        if (ob_get_level() === 0) {
...
add_action('shutdown', function() {
    ...
    // Only flush if we started the buffer
...
function ynm_add_studio_schema() {
    ...
    // Safety check: Make sure $post exists and has an ID
    ...
    // Safety check: Make sure we have a title
